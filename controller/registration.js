...
function showCourseDetails() {
  ...
  const selectedOption =
    courseSelect.value !== "none" ? courseSelect.value : "";
  ...
  for (const course in courseList) {
    ...
    if (
      courseCode.includes(selectedOption) &&
      (courseNumber === "" || courseNum.startsWith(courseNumber))
    ) {
...
searchResults.addEventListener("click", function (event) {
  if (event.target.classList.contains("select-course-btn")) {
    ...
    card.innerHTML = `
        <div class="card custom-card" id="${courseCode} ${courseNum}">
          <div class="card-body">
            ...
            <ul class="list-group list-group-flush">
              ...
              <li class="list-group-item"><button class="${courseCode} remove-course-btn" id="${courseCode} ${courseNum}-remove">Remove</button></li>
            </ul>
          </div>
        </div>
      `;
    ...
    selectedCoursesBox.addEventListener("click", function(event) {
